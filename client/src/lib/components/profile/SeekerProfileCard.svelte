	...
	<div class="flex items-start justify-between mb-6 border-b border-gray-200 pb-6">
		...
		{#if !isPreviewMode && showApplicationInfo}
			<div class="flex gap-2">
				<button 
					class="flex items-center px-4 py-2 text-sm bg-white rounded-md border border-gray-200 hover:bg-gray-50"
					on:click={onNotes}
				>
					<StickyNote class="w-4 h-4 mr-2" />Notes
				</button>
				<button 
					class="flex items-center px-4 py-2 text-sm bg-green-600 font-medium text-white rounded-md hover:bg-green-700 disabled:opacity-50"
					on:click={() => onAccept(candidateId)}
					disabled={isUpdatingStatus || candidateStatus === 'Accepted' || candidateStatus === 'Rejected'}
				>
					<Check class="w-4 h-4 mr-2" />Accept
				</button>
				<button 
					class="flex items-center px-4 py-2 text-sm bg-red-500 font-medium text-white rounded-md hover:bg-red-600 disabled:opacity-50"
			...
			<div>
				...
				{#if userData.linkedin && userData.linkedin.trim()}
					<div class="flex items-center gap-1">
						...
						<p class="text-gray-900 text-sm">{userData.linkedin}</p>
					</div>
				{:else}
			...
			<div>
				...
				{#if userData.github && userData.github.trim()}
					<div class="flex items-center gap-1">
						...
						<p class="text-gray-900 text-sm">{userData.github}</p>
					</div>
				{:else}
					<p class="text-gray-900 text-sm">-</p>
				{/if}
			...
			<div>
				...
				{#if userData.portfolio && userData.portfolio.trim()}
					<div class="flex items-center gap-1">
						...
						<p class="text-gray-900 text-sm">{userData.portfolio}</p>
					</div>
				{:else}
					<p class="text-gray-900 text-sm">-</p>
				{/if}
