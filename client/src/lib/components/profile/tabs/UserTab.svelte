					...
					<button 
						onclick={() => {
							if (isGoogleConnected) {
								...
								// Handle connect logic - redirect to OAuth
								window.location.href = `${import.meta.env.VITE_BACKEND}/auth/google?role=login`;
							}
						}}
						class="px-3 py-1.5 text-xs font-medium border rounded-md transition-colors cursor-pointer {isGoogleConnected ? 'text-red-600 border-red-200 hover:bg-red-50' : 'text-green-600 border-green-200 hover:bg-green-50'}"
					>
