...
// Test 9: Ensure header values are sanitized to prevent header injection
func TestFileDownloadHeaderSanitization(t *testing.T) {
	setupFileTestData(t)
	...
	// Malicious-looking filename and content-type (contains CRLF)
	maliciousFilename := "goodname.pdf\r\nInjected-Header: evil"
	maliciousContentType := "application/pdf\r\nInjected-Header: evil"
	...
	// Content-Type must not contain CR or LF
	ct := w.Header().Get("Content-Type")
	assert.NotContains(t, ct, "\r")
	assert.NotContains(t, ct, "\n")

	// Content-Disposition header should not include injected header text
	cd := w.Header().Get("Content-Disposition")
	assert.NotContains(t, cd, "\r")
	assert.NotContains(t, cd, "\n")
	// Ensure sanitized filename present and malicious part removed
	assert.Contains(t, cd, "goodname.pdf")
	assert.NotContains(t, cd, "Injected-Header")
}
